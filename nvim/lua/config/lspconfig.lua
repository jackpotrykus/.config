...
local on_attach = function(_, bufnr)
    ...
    vim.api
        .nvim_buf_set_keymap(bufnr, 'n', '<leader>i', '<cmd>lua vim.lsp.diagnostic.show_line_diagnostics()<CR>', opts)
...
local servers = {
    'pyright', -- 'pylsp',
    'r_language_server', 'clangd', 'ccls', 'gopls', 'texlab', 'bashls', 'vimls', 'java_language_server', 'ocamllsp'
}
...
nvim_lsp.efm.setup {
    ...
    settings = {
        ...
        languages = {
            lua = {
                {
                    formatCommand = 'lua-format -i --no-keep-simple-function-one-line --no-break-after-operator --column-limit=120 --break-after-table-lb --indent-width=2',
                    formatStdin = true
                }
            }
        }
